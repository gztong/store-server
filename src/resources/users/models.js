...
class User {
     ...
     * a) Prescription doesn't have ID: Create an UUID for it and add it to the array
     * b) Prescription has ID: Find it in the array and, if exists, update it. If not, throw error
     */
    @DBDecorators.table(tables.User)
    static async updatePrescription(userId, prescription) {
        log.debug('update rethinkdb for prescription');
        if (!prescription.id) {
            log.debug({userId, prescription}, 'Adding user prescription');
            let id = await rethinkdb.uuid();
            await this.table.get(userId).update({
                prescriptions: rethinkdb.row('prescriptions').append(Object.assign({id: id}, prescription)),
                updatedAt: new Date()
            }).run();
        } else {
            log.debug({userId, prescription}, 'Updating user prescription');
            let user = await User.get(userId);
            let prescriptions = user.prescriptions.map(function (userPrescription) {
                return (userPrescription.id !== prescription.id) ? userPrescription : prescription;
            });
            await this.table.get(userId).update({
    ...
    /**
     * Update the user's prescriptions array
     */
    @DBDecorators.table(tables.User)
    static async updatePrescriptions(userId, prescriptions) {
