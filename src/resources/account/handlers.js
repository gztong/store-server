...
class AccountHandlers {
    ...
    static async patch(request, reply) {
        ...
        if (hasKeys(request.payload, ['prescription'])) {
            user = await User.updatePrescription(userId, request.payload.prescription);

        }
        else if (hasKeys(request.payload, ['prescriptions'])) {
            user = await User.updatePrescriptions(userId, request.payload.prescriptions);
        }
        else if (hasKeys(request.payload, ['name'])) {
